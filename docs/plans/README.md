...
| `NEXT-PROMPT-v011-queue-phase2.md` | **pending** | Fermer #49 (la couture de conditionnement, deux relectures adverses, une troisieme recommandee), puis la phase 2 du budget de file, la stack de demo Prometheus + Grafana, et les cinq verdicts adoptes. Le critere d'acceptation de la phase 2 est un chiffre : le balayage de 19 collecteurs a plafond 1, ou la phase 1 a laisse 14 affames. |
| `NEXT-PROMPT-v010-field-work.md` | **done** (PRs #39, #40, #43, #44, #46, #47, v0.10.0) | The rest of v0.10: the systemd unit that cannot start a multi-instance build, the queue budget (the only seam, two-phase), the boot storm and the lifecycle that blocks its obvious fix, and four pieces of knowledge the templates never taught. `ROADMAP.md` v0.10 is the specification; this is the working order and the traps. Two limits landed with it and are stated in `CHANGELOG.md` rather than left implied: the queue budget is phase 1 only, and the measured sweep still starves 14 of 19 collectors at a ceiling of 1; the systemd fix covers systemd, not the container path. |
| `NEXT-PROMPT-field-defects.md` | **done** (PRs #32-#36, v0.9.0) | The four groups of defects the friction log produced, ordered by value over cost: the TLS/proxy gaps, the naming questions `/add-collector` keeps re-asking, the journal section that rots, `promtool` in `make check` (cost it before writing anything), and the fact that a scaffolded exporter is never told which version to start at. |
