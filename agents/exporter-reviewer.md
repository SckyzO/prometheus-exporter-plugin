...
| Lint 0 | Step 1's `make check` | — |
| `make docs-check` | Step 1 | — |
...
| CI-local green (`make check`) | Step 1 | — |
...
- **OpenMetrics stays on.** Confirm `cmd/*/main.go`'s `promhttp.HandlerFor`
  call still sets `EnableOpenMetrics: true` — easy to lose in a hand-edit
  of `main.go`, and nothing else in `make check` would catch it.
...
1. **`<name>Data`/fetch** — the *only* I/O in the file (an HTTP call
...
- If not, one bullet per gap: **file:line**, what's wrong, and a concrete
  fix — never "consider reviewing this," an actual suggested change.
