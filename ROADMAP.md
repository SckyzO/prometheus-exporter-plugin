...
- **Teach `multi-instance` in the references, delivered.** The target model
  shipped in v0.5 but appeared in none of the eleven reference documents that
  existed then, nor in `SKILL.md`'s step 0 walkthrough, so a session that
  learned from `references/` alone still believed there were two target
  models. v0.5's per-module credentials work had added it in two places
  as a side effect; the rest was an unpaid documentation debt from the model's
  own drop, and it is paid here.
